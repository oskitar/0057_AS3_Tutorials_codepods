package es.cerocerocincuentaysiete.util
{
	 ...
	 *
	 */
	public class Sierra24ADitherBitmapData implements IDitherBitmap
	{
		...
		function Sierra24ADitherBitmapData()
		{
		}
		...
		public function dither(bitmapData : BitmapData) : void
		{
			...
			var dispersionArray : Array = [2 / 4, 1 / 4, 1 / 4];
			...
			for (var y : int = 0; y < bitmapData.height;y++)
			{
				for (var x : int = 0; x <= bitmapData.width;x++)
				{
					...
					// (X+1, Y)
					// Capturamos el color del pixel x+1 para aplicarle el valor de dispersión de cuantización (2/4)
					...
					// (X-1, Y+1)
					// Capturamos el color del pixel x+1 para aplicarle el valor de dispersión de cuantización (1/4)
